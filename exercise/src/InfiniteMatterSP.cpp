...
double P_group_t::CorrelationEnergyIMSRG(InfiniteMatterSP *InfSP,
	double deltaS)
{
	
	return IMSRGInf(this, InfSP, deltaS).Iterate();

	for(const auto &t : co_enVec){
		if(isSimilarP(P, t.P)) return t.corr_en;
	}
	double corr_en = IMSRGInf(this, InfSP, deltaS).Iterate();
	co_enVec.emplace_back(P, corr_en);
	
	return corr_en;
}

double InfiniteMatterSP::CorrelationEnergy(double deltaS)
{
	const int n = 4*fNmax + 1;
	double corr_en = 0.;

	P_group_t * p = P(-2, 0, 0);
	p->CorrelationEnergyIMSRG(this, deltaS);
	return 0.;
